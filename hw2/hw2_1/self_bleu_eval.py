...
if __name__ == "__main__" :
    ...
    with open('score.txt', 'a+') as f2:
        for item in test:
            ...
            f2.write(item['id']+','+str(score_per_video[0])+'\n')
    average = sum(bleu) / len(bleu)

    os.system('rm score_history.txt')
    print("Average bleu score is " + str(average))
    with open('score_history.txt', 'a+') as f:
        f.write(' BLEU '+ str(average)+ '\n')
    
    sh= np.load('score_history.npy')
    sh= np.append(sh, average)
    np.save('score_history.npy', sh)
