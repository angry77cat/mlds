...
class ImageData(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        x = self.transforms(x)
        y = self.index_label[self.id_list[idx]]
        return x, y
    ...
    def _one_hot_labeling(self):
        ...
        for label in self.label_list[:]:
            ...
            self.index_label.append([index])
            # self.one_hot_label.append([0] * (index) + [1] + [0] * (n_comb-index-1))
        # print(self.one_hot_label[0])
        self.index_label = torch.LongTensor(self.index_label)
        # self.one_hot_label = torch.LongTensor(self.one_hot_label)
